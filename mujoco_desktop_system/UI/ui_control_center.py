...
class ControlCenter(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        notebook.add(self._build_vla_tab(notebook), text="多模态策略")
    ...
    def _build_vla_tab(self, parent: ttk.Notebook) -> ttk.Frame:
        ...
        train = ttk.LabelFrame(frame, text="多模态策略 SFT / 后训练")
        ...
        ttk.Button(train, text="启动多模态策略训练", command=self.start_vla_train).pack(anchor="w", pady=(4, 0))
        ...
        deploy = ttk.LabelFrame(frame, text="多模态策略部署")
        ...
        ttk.Button(deploy, text="启动多模态策略推理", command=self.start_vla_deploy).pack(anchor="w", pady=(4, 0))
        return frame
    ...
    def start_vla_collect(self) -> None:
        ...
        if self.vla_collect_show_var.get():
            cmd.append("--show")
        self._start_run(RunSpec("多模态策略采集", cmd))
    ...
    def start_vla_train(self) -> None:
        ...
        self._start_run(RunSpec("多模态策略训练", cmd))
    ...
    def start_vla_deploy(self) -> None:
        ...
        if self.vla_deploy_show_var.get():
            cmd.append("--show")
        self._start_run(RunSpec("多模态策略推理", cmd))
