...
class SimulationAutomationMixin:
    def _setup_automation_tab(self, tab_widget):
        ...
        vla_group = QtWidgets.QGroupBox("多模态策略训练数据采集")
        ...
        self.vla_record_cb = QtWidgets.QCheckBox("启用多模态策略采集")
    ...
    def start_auto_navigation(self):
        ...
        if not registration_ready:
            ...
            return
        self.auto_nav_tracking_path, entry_index, entry_distance = (
            build_centerline_entry_path(
                path,
                current_position,
                self._auto_param("point_threshold"),
                self._auto_param("centerline_tolerance"),
            )
        )
        if entry_distance > self._auto_param("centerline_tolerance") * 1.10:
    ...
    def choose_vla_save_directory(self):
        try:
            directory = QtWidgets.QFileDialog.getExistingDirectory(
                self, "选择多模态策略数据保存目录", self.vla_save_path_edit.text().strip() or PROJECT_ROOT
            )
            if directory:
        ...
        try:
            if self.auto_nav_state == "running":
                ...
                if action is None:
                    ...
                    if waypoint_reached:
                        path = self._get_auto_navigation_path()
                        # Preserve strict sequential confirmation, but catch up
                        # adjacent 1 mm samples within the same UI frame. The
                        # physical slider can advance several samples per frame;
                        # limiting this loop to one left the yellow target tens
                        # of millimetres behind the real tip and commanded a turn
                        # back toward an already-passed bifurcation.
                        for _ in range(16):
                            ...
                            self.auto_nav_passed_count = self.auto_nav_target_index
                            action = self._compute_auto_navigation_action()
                            if not bool(action.get("waypoint_reached", False)):
                                break
                        self._refresh_auto_navigation_guides()
                        self.auto_nav_action = action
                        self._update_navigation_target_window(action)
    ...
    def start_vla_episode(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "多模态策略采集启动失败", str(e))
    ...
    def _save_vla_image(self, category, image):
        ...
        try:
            ...
            print(f"多模态策略图像保存失败 ({category}): {e}")
        return None
